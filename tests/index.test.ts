...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['DAM_STATUS_API_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: DamStatusAPI) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['DAM_STATUS_API_LOG'] = 'debug';
      const client = new DamStatusAPI({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['DAM_STATUS_API_LOG'] = 'not a log level';
      const client = new DamStatusAPI({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'DAM_STATUS_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    test('no trailing slash', () => {
      const client = new DamStatusAPI({ baseURL: 'http://localhost:5000/custom/path', apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null)).toEqual('http://localhost:5000/custom/path/foo');
    });

    afterEach(() => {
      process.env['DAM_STATUS_API_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new DamStatusAPI({ baseURL: 'https://example.com', apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new DamStatusAPI({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new DamStatusAPI({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['DAM_STATUS_API_API_KEY'] = 'My API Key';
    const client = new DamStatusAPI();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['DAM_STATUS_API_API_KEY'] = 'another My API Key';
    const client = new DamStatusAPI({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new DamStatusAPI({ apiKey: 'My API Key', fetch: testFetch });
