...
import { ClientOptions } from 'poc-dam-status-api';

const prompt = `Runs JavaScript code to interact with the Dam Status API API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    DAM_STATUS_API_API_KEY: requireValue(
      readEnv('DAM_STATUS_API_API_KEY') ?? client.apiKey,
      'set DAM_STATUS_API_API_KEY environment variable or provide apiKey client option',
    ),
    DAM_STATUS_API_SC_DAM: readEnv('DAM_STATUS_API_SC_DAM') ?? client.scDam ?? undefined,
    DAM_STATUS_API_BASE_URL: readEnv('DAM_STATUS_API_BASE_URL') ?? client.baseURL ?? undefined,
  };
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = 'poc-dam-status-api';
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
        ...
        ...(client.scDam != null ? { scDam: client.scDam } : undefined),
        defaultHeaders: {
