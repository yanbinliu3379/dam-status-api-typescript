...
import { DamStatusAPI, ClientOptions } from 'poc-dam-status-api';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: DamStatusAPI)`
    : `const run: (${functionSource.client}: DamStatusAPI) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(['client.status.check'], { threshold: 1, shouldSort: true });
